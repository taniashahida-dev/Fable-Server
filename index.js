...
async function run() {
  try {
    ...
    app.get("/api/admin/raw-analytics", verifyToken, verifyAdmin, async (req, res) => {
      try {
    ...
    app.get('/api/writer/check-verification/:writerId', verifyToken, async (req, res) => {
      try {
    ...
    // Writer Purchased/Sales History
    app.get("/api/purchased-books/writer", verifyToken, verifyWriter, async (req, res) => {
      try {
    ...
    app.get("/api/purchased-books/reader", verifyToken, async (req, res) => {
      try {
    ...
    app.post("/api/bookmarks", verifyToken, async (req, res) => {
      try {
        ...
        if (userEmail !== req.user.email) return res.status(403).send({ error: true, message: "forbidden access" });
    ...
    // Get All Ebooks with Search, Filtering & Sorting
    ...
    // API Route to fetch metadata of a specific standalone ebook document (Duplicate Cleaned)
    app.get('/api/ebooks/:id', async (req, res) => {
      try {
        ...
        if (userEmail) {
          const purchaseRecord = await purchasedBookCollection.findOne({
            bookId: id,
          ...
          const bookmarkRecord = await bookmarkCollection.findOne({
            bookId: id,
            userEmail: userEmail 
          });
